...
- [Agent AFK](https://github.com/griffinwork40/agent-afk) - Apache-2.0 TypeScript coding-agent harness for away-from-keyboard async runs: a cron-friendly headless daemon and Telegram bridge keep long tasks alive, every turn resolves to an explicit terminal state (Done/Blocked/Asking/Interrupted) used as the async hand-off artifact, and the loop itself — prompts, permission gates, model routing, lifecycle hooks, MCP client, transitive-abort graph, append-only trace — is editable code over a provider-agnostic event stream (Anthropic Messages + OpenAI-compatible, including local MLX/llama.cpp runners).
